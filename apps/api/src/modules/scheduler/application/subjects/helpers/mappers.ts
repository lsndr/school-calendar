...
export function mapRecurrenceToDto(
  recurrence:
    DailyRecurrence | WeeklyRecurrence | BiWeeklyRecurrence | MonthlyRecurrence,
):
  | DailyRecurrenceDto
  | WeeklyRecurrenceDto
  | BiWeeklyRecurrenceDto
...
export function mapRecurrenceToDto(
  ...
  } else if (recurrence.type === RecurrenceType.BiWeekly) {
...
export function mapRawRecurrenceToDto(
  type: unknown,
  data: unknown,
):
  | DailyRecurrenceDto
  ...
  if (type === 'daily') {
    ...
    return new WeeklyRecurrenceDto(data as Omit<WeeklyRecurrenceDto, 'type'>);
  } else if (type === 'biweekly') {
    return new BiWeeklyRecurrenceDto(
      data as Omit<BiWeeklyRecurrenceDto, 'type'>,
    );
  } else if (type === 'monthly') {
    return new MonthlyRecurrenceDto(data as Omit<MonthlyRecurrenceDto, 'type'>);
  } else {
    throw new Error('Unknown recurrence type');
  }
}
